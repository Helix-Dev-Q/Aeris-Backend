...
app.post("/api/v2/discovery/surface/*", async (req, res) => {
  const discovery = JSON.parse(
    fs.readFileSync("./src/data/Discovery/Discovery.json", "utf-8")
  );
...
app.get("/api/v1/public/tags", (req, res) => {
  const tagList = JSON.parse(
    fs.readFileSync("./src/data/PlayerTags/Tags.json", "utf-8")
  );
...
app.post("/api/v1/assets/Fortnite/*/*", async (req, res) => {
  ...
  if (FortCreativeDiscoverySurface === 0) {
    const discovery_api_assets = require("../data/Discovery/Creative.json");
    ...
    if (value.name.startsWith("BRSeason")) {
      if (!Number.isNaN(Number(value.name.split("BRSeason")[1]))) {
        ...
        if (offer) {
          ...
          var BattlePass = require(`../data/BattlePass/Jsons/${Season}.json`);
...
app.post(
  ...
  async (req, res) => {
    ...
    if (!profile.items[activeLoadoutId]) {
      activeLoadoutId = profile.stats.attributes.loadouts[
        profile.stats.attributes.active_loadout_index
      ] || "sandbox_loadout";
    }
...
app.post(
  ...
  async (req, res) => {
    ...
    // Ensure the locker item actually exists; fall back to sandbox_loadout
    if (!profile.items[activeLoadoutId]) {
      activeLoadoutId = profile.stats.attributes.loadouts[
        profile.stats.attributes.active_loadout_index
      ] || "sandbox_loadout";
      ...
      case "CopyCosmeticLoadout":
        try {
          ...
          if (req.body.sourceIndex == 0) {
            item = profile.items[`Aeris${req.body.targetIndex - 1}-loadout`];
            if (!item) {
              profile.items[`Aeris${req.body.targetIndex - 1}-loadout`] =
            ...
            if (req.body.optNewNameForTarget != "")
              profile.items[
                `Aeris${req.body.targetIndex - 1}-loadout`
              ].attributes["locker_name"] = req.body.optNewNameForTarget;
            profile.stats.attributes.loadouts[req.body.targetIndex] = `Aeris${
              req.body.targetIndex - 1
            }-loadout`;
            profile.stats.attributes["active_loadout_index"] =
              req.body.targetIndex;
            profile.stats.attributes["last_applied_loadout"] = `Aeris${
              req.body.targetIndex - 1
            }-loadout`;
            profile.items["sandbox_loadout"].attributes["locker_slots_data"] =
              item2.attributes["locker_slots_data"];
          } else {
            item = profile.items[`Aeris${req.body.sourceIndex - 1}-loadout`];
            if (!item)
              return Utils.createError(
                "errors.com.epicgames.modules.profiles.operation_forbidden",
            ...
            profile.items[`Aeris${req.body.sourceIndex - 1}-loadout`] =
              profile.items["sandbox_loadout"];
            profile.items[
              `Aeris${req.body.sourceIndex - 1}-loadout`
      ...
      case "DeleteCosmeticLoadout":
        try {
          ...
          if (req.body.fallbackLoadoutIndex != -1) {
            item = profile.items[`Aeris${fallbackLoadoutIndex - 1}-loadout`];
            if (!item)
              return Utils.createError(
                "errors.com.epicgames.modules.profiles.operation_forbidden",
                `Locker item {0} not found`,
            ...
            profile.items["sandbox_loadout"].attributes["locker_slots_data"] =
              item.attributes["locker_slots_data"];
            let n = item.attributes["locker_name"];
            profile.items[`Aeris${fallbackLoadoutIndex - 1}-loadout`] =
              profile.items["sandbox_loadout"];
            profile.items[
              `Aeris${fallbackLoadoutIndex - 1}-loadout`
            ].attributes["locker_name"] = n;
          }
