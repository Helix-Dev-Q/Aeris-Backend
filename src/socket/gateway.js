...
wss.on("listening", () => {
  logger.xmpp(`Aeris Xmpp Services Running`);
});
wss.on("connection", async (ws, req) => {
  ...
  const pingInterval = setInterval(() => {
    if (ws.readyState !== ws.OPEN) return clearInterval(pingInterval);
      ...
      case "iq":
        switch (msg.root.attributes.id) {
          case "_xmpp_bind1":
            ...
            // register client immediately after bind so session1 works
            ...
            break;
          case "_xmpp_session1":
            if (!jid || !accountId) return Error(ws);
            ws.send(
              XMLBuilder.create("iq")
                .attribute("to", jid)
                .attribute("from", domain)
          ...
          default:
            ...
            // respond to XMPP pings (urn:xmpp:ping)
            if (msg.root.children?.find(c => c.name === 'ping')) {
              ws.send(
                XMLBuilder.create("iq")
                  .attribute("to", jid)
                  .attribute("from", domain)
