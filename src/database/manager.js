...
async function createProfiles(accountId) {
  ...
  let profiles = {};
  fs.readdirSync(path.join(__dirname, "../templates/DefaultProfiles/")).forEach((fileName) => {
    const profile = JSON.parse(fs.readFileSync(path.join(__dirname, `../templates/DefaultProfiles/${fileName}`), "utf-8"));
