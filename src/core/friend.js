...
class friendsClass {
  ...
  async sendFriendReq(fromId, toId) {
    ...
    Utils.SendXMPPMessage({ payload: { accountId: to?.accountId, status: "PENDING", direction: "OUTBOUND", created: new Date().toISOString(), favorite: false }, type: "com.epicgames.friends.core.apiobjects.Friend", timestamp: new Date().toISOString() }, from?.accountId);
    toFriends.incoming.push({ accountId: from?.accountId, created: new Date().toISOString() });
    Utils.SendXMPPMessage({ payload: { accountId: from?.accountId, status: "PENDING", direction: "INBOUND", created: new Date().toISOString(), favorite: false }, type: "com.epicgames.friends.core.apiobjects.Friend", timestamp: new Date().toISOString() }, to?.accountId);
  ...
  async acceptFriendReq(fromId, toId) {
    ...
    if (incomingIndex != -1) {
      ...
      Utils.SendXMPPMessage({ payload: { accountId: to?.accountId, status: "ACCEPTED", direction: "OUTBOUND", created: new Date().toISOString(), favorite: false }, type: "com.epicgames.friends.core.apiobjects.Friend", timestamp: new Date().toISOString() }, from?.accountId);
      ...
      Utils.SendXMPPMessage({ payload: { accountId: from?.accountId, status: "ACCEPTED", direction: "OUTBOUND", created: new Date().toISOString(), favorite: false }, type: "com.epicgames.friends.core.apiobjects.Friend", timestamp: new Date().toISOString() }, to?.accountId);
      await from?.updateOne({ $set: { list: fromFriends } });
      await to?.updateOne({ $set: { list: toFriends } });
    }
    return true;
  ...
  async deleteFriend(fromId, toId) {
    ...
    for (let listType in fromFriends) {
      ...
      if (findFriend != -1) { fromFriends[listType].splice(findFriend, 1); removed = true; }
      if (listType == "blocked") continue;
      if (findToFriend != -1) toFriends[listType].splice(findToFriend, 1);
    }
    if (removed == true) {
      Utils.SendXMPPMessage({ payload: { accountId: to?.accountId, reason: "DELETED" }, type: "com.epicgames.friends.core.apiobjects.FriendRemoval", timestamp: new Date().toISOString() }, from?.accountId);
      Utils.SendXMPPMessage({ payload: { accountId: from?.accountId, reason: "DELETED" }, type: "com.epicgames.friends.core.apiobjects.FriendRemoval", timestamp: new Date().toISOString() }, to?.accountId);
      await from?.updateOne({ $set: { list: fromFriends } });
      await to?.updateOne({ $set: { list: toFriends } });
    }
    return true;
